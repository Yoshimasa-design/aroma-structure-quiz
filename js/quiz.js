...
function answer(o,btn){if(locked)return;locked=true;const a=queue[i],ok=o.id===a.id;if(ok)score++;addResult(a.id,ok,mode);document.querySelectorAll(".choice").forEach(b=>{b.disabled=true;if(b.dataset.id===a.id)b.classList.add("correct")});if(!ok)btn.classList.add("wrong");const r=$("#result");theme(r,a);r.classList.remove("hidden");r.innerHTML=`<h2>${ok?"正解です":"正解は"}：${a.name_ja}</h2><span class="tag">${a.class_group}</span><p>${a.odor}</p><div class="actions">${favoriteButtonMarkup(a.id)}${mode==="odor"?'<button class="btn secondary" id="showDetail" type="button">図鑑で見る</button>':""}<button class="btn primary" id="next">${i===queue.length-1?"結果を見る":"次へ"}</button></div>`;bindFavoriteButtons(r);if(mode==="odor")$("#showDetail").onclick=()=>detailModal.open(a);$("#next").onclick=next}
...
async function init(){
 ...
 if(mode==="odor")detailModal=createDetailModal();
